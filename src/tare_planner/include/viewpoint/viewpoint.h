...
public:
  ...
  template <class PCLPointType> void UpdateCoverage(const PCLPointType &point) {
    lidar_model_.UpdateCoverage<PCLPointType>(point);
  }
  template <class PCLPointType>
  bool CheckVisibility(const PCLPointType &point,
                       double occlusion_threshold) const {
    return lidar_model_.CheckVisibility<PCLPointType>(point,
                                                      occlusion_threshold);
  }
  ...
  void SetInCollision(bool in_collision) { in_collision_ = in_collision; }
  ...
  void SetCandidate(bool candidate) { is_candidate_ = candidate; }
  ...
  void SetCellInd(int cell_ind) { cell_ind_ = cell_ind; }
  int GetCellInd() const { return cell_ind_; }

  int GetCellIndex() const { return cell_ind_; }

  void
  GetVisualizationCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &vis_cloud) const {
...
private:
  ...
  // Whether this viewpoint and the robot’s current location are within the same
  // connected component. It must be true to have a collision-free path planned
  // from the current robot position to this viewpoint.
  bool connected_;
  // Whether this viewpoint has been visited by the robot. If true, its coverage
  // area will not be updated and it will not be selected in the sampling
  // process.
  bool visited_;
