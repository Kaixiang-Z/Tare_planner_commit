...
public:
  ...
  void UpdateRobotPosition(geometry_msgs::Point robot_position) {
    bool pointcloud_manager_rolling =
    ...
    rolling_occupancy_grid_->InitializeOrigin(
    ...
    if (pointcloud_manager_rolling) {
      ...
      rolled_in_occupancy_cloud_->cloud_ =
          pointcloud_manager_->GetRolledInOccupancyCloud();
      pointcloud_manager_->ClearNeighborCellOccupancyCloud();
      // rolled_in_occupancy_cloud_->Publish();
      rolling_occupancy_grid_->UpdateOccupancyStatus(
          rolled_in_occupancy_cloud_->cloud_);
    }
    if (occupancy_grid_rolling) {
      ...
      pointcloud_manager_->StoreOccupancyCloud(
          rolled_out_occupancy_cloud_->cloud_);

      pointcloud_manager_->GetOccupancyCloud(
          pointcloud_manager_occupancy_cloud_->cloud_);
  ...
  void
  UpdateRegisteredCloud(typename pcl::PointCloud<PCLPointType>::Ptr &cloud) {
  ...
  void UpdateKeyposeCloud(
    ...
    if (keypose_cloud->points.empty()) {
      ...
      if (parameters_.kUseCoverageBoundaryOnObjectSurface) {
        GetCoverageCloudWithinBoundary<PlannerCloudPointType>(
      ...
      *(stacked_cloud_->cloud_) += *(keypose_cloud_->cloud_);
      stacked_cloud_downsizer_.Downsize(stacked_cloud_->cloud_,
                                        parameters_.kSurfaceCloudDwzLeafSize,
                                        parameters_.kSurfaceCloudDwzLeafSize,
                                        parameters_.kSurfaceCloudDwzLeafSize);
      for (const auto &point : stacked_cloud_->cloud_->points) {
        if (point.r < 40) // TODO: computed from the keypose cloud resolution
                          // and stacked cloud resolution
        {
      ...
      *vertical_surface_cloud_stack_[keypose_cloud_count_] =
      ...
      stacked_cloud_downsizer_.Downsize(stacked_vertical_surface_cloud_->cloud_,
                                        parameters_.kSurfaceCloudDwzLeafSize,
                                        parameters_.kSurfaceCloudDwzLeafSize,
                                        parameters_.kSurfaceCloudDwzLeafSize);
  ...
  template <class PCLPointType>
  void GetCoverageCloudWithinBoundary(
  ...
  void
  GetUncoveredArea(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>
                       &viewpoint_manager,
  ...
  void
  GetVisualizationPointCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr vis_cloud);
...
private:
  ...
  pointcloud_utils_ns::PointCloudDownsizer<PlannerCloudPointType>
      stacked_cloud_downsizer_;
  pointcloud_utils_ns::PointCloudDownsizer<pcl::PointXYZI>
      collision_cloud_downsizer_;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<PlannerCloudPointType>>
  ...
  std::unique_ptr<rolling_occupancy_grid_ns::RollingOccupancyGrid>
