...
public:
  ...
  void SetViewPointManager(
      std::shared_ptr<viewpoint_manager_ns::ViewPointManager> const
          &viewpoint_manager) {
  ...
  exploration_path_ns::ExplorationPath SolveLocalCoverageProblem(
      ...
      int uncovered_point_num, int uncovered_frontier_point_num = 0);

  // Runtime
  int GetFindPathRuntime() { return find_path_runtime_; }
  int GetViewPointSamplingRuntime() { return viewpoint_sampling_runtime_; }
  int GetTSPRuntime() { return tsp_runtime_; }

  bool IsLocalCoverageComplete() { return local_coverage_complete_; }

  // Visualization
  void
  GetSelectedViewPointVisCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud);

private:
  int GetBoundaryViewpointIndex(
      const exploration_path_ns::ExplorationPath &global_path);
  void
  GetBoundaryViewpointIndices(exploration_path_ns::ExplorationPath global_path);
  void GetNavigationViewPointIndices(
