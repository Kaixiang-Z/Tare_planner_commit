...
public:
  ...
  explicit LiDARModel(double px = 0.0, double py = 0.0, double pz = 0.0,
                      double rw = 1.0, double rx = 0.0, double ry = 0.0,
                      double rz = 0.0);
   ...
   * @param point
   */
  template <class PointType> void UpdateCoverage(const PointType &point) {
    double distance_to_point =
        misc_utils_ns::PointXYZDist<PointType, geometry_msgs::Point>(
            point, pose_.position);
  ...
  bool CheckVisibility(const PointType &point,
                       double occlusion_threshold) const {
    double distance_to_point =
        misc_utils_ns::PointXYZDist<PointType, geometry_msgs::Point>(
            point, pose_.position);
    ...
    for (int n = -horizontal_neighbor_num; n <= horizontal_neighbor_num; n++) {
      ...
      for (int m = -vertical_neighbor_num; m <= vertical_neighbor_num; m++) {
        ...
        if ((!isZero(previous_distance_to_point) &&
             distance_to_point <
                 previous_distance_to_point + occlusion_threshold &&
             !reset_[ind]) ||
            reset_[ind]) {
          return true;
        }
   ...
   * @param max_range
   */
  void GetVisualizationCloud(
      pcl::PointCloud<pcl::PointXYZI>::Ptr &visualization_cloud,
  ...
  void SetHeight(double height) { pose_.position.z = height; }
...
private:
   ...
   * @return false
   */
  bool isZero(double x) const { return std::abs(x) < kEpsilon; }
  ...
  inline int GetHorizontalAngle(double dx, double dy) const {
    double horizontal_angle =
        (misc_utils_ns::ApproxAtan2(dy, dx) * kToDegreeConst + 180) /
  ...
  inline int GetHorizontalNeighborNum(double distance_to_point) const {
    return static_cast<int>(ceil(pointcloud_resolution_ / distance_to_point *
                                 kToDegreeConst / kHorizontalResolution)) /
           2;
  }
  /**
  ...
  inline int GetVerticalNeighborNum(double distance_to_point) const {
    return static_cast<int>(ceil(pointcloud_resolution_ / distance_to_point *
                                 kToDegreeConst / kVerticalResolution)) /
           2;
  }
  ...
  static const int kHorizontalVoxelSize =
      kHorizontalFOV / kHorizontalResolution;
  ...
  // The distance that a ray can reach from the direction determined by the
