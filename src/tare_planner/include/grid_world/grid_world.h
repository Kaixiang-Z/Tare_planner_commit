...
public:
  ...
  CellStatus GetStatus() { return status_; }
  void SetStatus(CellStatus status) { status_ = status; }
  ...
  geometry_msgs::Point GetPosition() { return center_; }
  ...
  bool IsRobotPositionSet() { return robot_position_set_; }
  geometry_msgs::Point GetRobotPosition() { return robot_position_; }
  void AddVisitCount() { visit_count_++; }
  int GetVisitCount() { return visit_count_; }
  void Reset();
  int GetKeyposeID() { return keypose_id_; }
  ...
  void
  SetRoadmapConnectionPoint(const Eigen::Vector3d &roadmap_connection_point) {
  ...
  bool IsPathAddedToKeyposeGraph() { return path_added_to_keypose_graph_; }
  void SetPathAddedToKeyposeGraph(bool add_path) {
    path_added_to_keypose_graph_ = add_path;
  }
  bool IsRoadmapConnectionPointSet() { return roadmap_connection_point_set_; }
...
private:
  ...
  // Position of the robot where this cell is first observed and turned
  // EXPLORING
  ...
  // Whether this cell is in the planning horizon, which consists of nine cells
  // around the robot.
  bool in_horizon_;
...
public:
  ...
  int GetMinAddPointNum() { return kMinAddPointNumSmall; }
  int GetMinAddFrontierPointNum() { return kMinAddFrontierPointNum; }
  ...
  int sub2ind(const Eigen::Vector3i &sub) { return subspaces_->Sub2Ind(sub); }
  int sub2ind(int row_idx, int col_idx, int level_idx) {
    return subspaces_->Sub2Ind(row_idx, col_idx, level_idx);
  }
  Eigen::Vector3i ind2sub(int ind) { return subspaces_->Ind2Sub(ind); }
  ...
  bool IndInBound(int ind) { return subspaces_->InRange(ind); }
  ...
  void GetCellSub(int &row_idx, int &col_idx, int &level_idx, double qx,
                  double qy, double qz);
  ...
  bool UseKeyposeGraph() { return use_keypose_graph_; }
  ...
  void SetCellRobotPosition(int cell_ind,
  ...
  void
  UpdateCellStatus(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>
                       &viewpoint_manager);
  ...
  inline void
  SetCurKeyposeGraphNodePosition(geometry_msgs::Point node_position) {
  ...
  bool HomeSet() { return set_home_; }
  bool IsReturningHome() { return return_home_; }
  void
  GetCellViewPointPositions(std::vector<Eigen::Vector3d> &viewpoint_positions);
  void AddPathsInBetweenCells(
      const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>
          &viewpoint_manager,
      const std::unique_ptr<keypose_graph_ns::KeyposeGraph> &keypose_graph);
  bool PathValid(const nav_msgs::Path &path, int from_cell_ind,
                 int to_cell_ind);
