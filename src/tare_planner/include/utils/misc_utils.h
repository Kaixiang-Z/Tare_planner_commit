...
#define MY_ASSERT(val)                                                         \
  if (!(val)) {                                                                \
    std::cout << "\033[31m Error at [File: " << __FILE__                       \
              << "][Line: " << __LINE__ << "]"                                 \
...
void LinInterpPoints(const geometry_msgs::Point &p1,
                     const geometry_msgs::Point &p2, double resolution,
                     typename pcl::PointCloud<PCLPointType>::Ptr &cloud) {
  double point_dist =
      PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(p1, p2);
  if (point_dist < 0.01)
    return;
  PCLPointType point1;
  point1.x = p1.x;
 ...
 * @return
 */
template <class T> bool InRange(const std::vector<T> &list, int index) {
...
void PublishCloud(const ros::Publisher &cloud_publisher,
...
void Publish(const ros::Publisher &publisher, ROSMsgType &msg,
...
public:
  ...
  void Stop(bool show, std::string unit = "ms") {
    ...
    if (show) {
      if (unit == "ms") {
        std::cout << name_ << " takes "
                  << GetDuration<std::chrono::milliseconds>() << " ms"
                  << std::endl;
      } else if (unit == "us") {
        std::cout << name_ << " takes "
                  << GetDuration<std::chrono::microseconds>() << " us"
                  << std::endl;
      } else if (unit == "ns") {
        std::cout << name_ << " takes "
                  << GetDuration<std::chrono::nanoseconds>() << " ns"
                  << std::endl;
      } else if (unit == "s") {
        std::cout << name_ << " takes " << GetDuration<std::chrono::seconds>()
                  << " s" << std::endl;
      } else {
  ...
  template <class U> int GetDuration() {
  ...
  int GetDuration(std::string unit = "ms") {
    if (unit == "ms") {
      ...
      return std::chrono::duration_cast<std::chrono::seconds>(duration_)
          .count();
    } else {
      std::cout << "timer unit error!" << std::endl;
      return 0;
...
public:
  ...
  explicit Marker(ros::NodeHandle *nh, std::string pub_topic,
                  std::string frame_id)
  ...
  void Publish() {
    misc_utils_ns::Publish<visualization_msgs::Marker>(marker_pub_, marker_,
                                                       frame_id_);
  }
...
bool InRange(const Eigen::Vector3i &sub,
             const Eigen::Vector3i &max_sub = Eigen::Vector3i(INT_MAX, INT_MAX,
                                                              INT_MAX),
             const Eigen::Vector3i &min_sub = Eigen::Vector3i(INT_MIN, INT_MIN,
                                                              INT_MIN));
...
bool InFOVSimple(const Eigen::Vector3d &point_position,
                 ...
                 double xy_dist_threshold = 0, double z_diff_threshold = 0,
                 bool print = false);
...
bool AStarSearchWithMaxPathLength(
    ...
    double &shortest_dist, double max_path_length = DBL_MAX);
