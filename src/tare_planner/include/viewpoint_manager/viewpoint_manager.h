...
public:
  ...
  inline int GetViewPointNum() { return viewpoints_.size(); }
  Eigen::Vector3d GetResolution() { return vp_.kResolution; }
  inline void UpdateViewPointBoundary(const geometry_msgs::Polygon &polygon) {
    viewpoint_boundary_ = polygon;
  }

  inline void
  UpdateNogoBoundary(const std::vector<geometry_msgs::Polygon> &nogo_boundary) {
  ...
  void UpdateViewPointCoverage(
    ...
    // std::cout << "update viewpoint num: " << update_viewpoint_count <<
    // std::endl;
    for (const auto &point : cloud->points) {
      ...
      // for (auto& viewpoint : viewpoints_)
      {
        ...
        if (misc_utils_ns::InFOVSimple(
                ...
                vp_.kVerticalFOVRatio, vp_.kSensorRange,
                vp_.kInFovXYDistThreshold, vp_.kInFovZDiffThreshold)) {
    ...
    for (const auto &point : cloud->points) {
      for (const auto &viewpoint_ind : updated_viewpoint_indices_) {
        ...
        geometry_msgs::Point viewpoint_position =
            viewpoints_[array_ind].GetPosition();
        if (misc_utils_ns::InFOVSimple(
                Eigen::Vector3d(point.x, point.y, point.z),
                Eigen::Vector3d(viewpoint_position.x, viewpoint_position.y,
                                viewpoint_position.z),
  ...
  bool VisibleByViewPoint(const PointType &point, int viewpoint_ind) {
    ...
    bool visible = viewpoints_[array_ind].CheckVisibility<PointType>(
        point, vp_.kCoverageOcclusionThr);
  ...
  void SetViewPointConnected(int viewpoint_ind, bool connected,
                             bool use_array_ind = false);

  bool ViewPointVisited(int viewpoint_ind, bool use_array_ind = false);
  void SetViewPointVisited(int viewpoint_ind, bool visited,
                           bool use_array_ind = false);

  bool ViewPointSelected(int viewpoint_ind, bool use_array_ind = false);
  void SetViewPointSelected(int viewpoint_ind, bool selected,
                            bool use_array_ind = false);

  bool IsViewPointCandidate(int viewpoint_ind, bool use_array_ind = false);
  void SetViewPointCandidate(int viewpoint_ind, bool candidate,
                             bool use_array_ind = false);
  ...
  void SetViewPointHeight(int viewpoint_ind, double height,
                          bool use_array_ind = false);

  bool ViewPointInCurrentFrameLineOfSight(int viewpoint_ind,
                                          bool use_array_ind = false);
  void
  SetViewPointInCurrentFrameLineOfSight(int viewpoint_ind,
                                        bool in_current_frame_line_of_sight,
                                        bool use_array_ind = false);

  geometry_msgs::Point GetViewPointPosition(int viewpoint_ind,
                                            bool use_array_ind = false);
  void SetViewPointPosition(int viewpoint_ind, geometry_msgs::Point position,
                            bool use_array_ind = false);

  int GetViewPointCellInd(int viewpoint_ind, bool use_array_ind = false);
  void SetViewPointCellInd(int viewpoint_ind, int cell_ind,
                           bool use_array_ind = false);

  int GetViewPointCollisionFrameCount(int viewpoint_ind,
                                      bool use_array_ind = false);
  void AddViewPointCollisionFrameCount(int viewpoint_ind,
                                       bool use_array_ind = false);
  void ResetViewPointCollisionFrameCount(int viewpoint_ind,
                                         bool use_array_ind = false);

  void ResetViewPointCoveredPointList(int viewpoint_ind,
                                      bool use_array_ind = false);
  void AddUncoveredPoint(int viewpoint_ind, int point_ind,
                         bool use_array_ind = false);
  void AddUncoveredFrontierPoint(int viewpoint_ind, int point_ind,
                                 bool use_array_ind = false);
  const std::vector<int> &
  GetViewPointCoveredPointList(int viewpoint_ind,
                               bool use_array_ind = false) const;
  const std::vector<int> &
  GetViewPointCoveredFrontierPointList(int viewpoint_ind,
                                       bool use_array_ind = false) const;

  int GetViewPointCoveredPointNum(int viewpoint_ind,
                                  bool use_array_ind = false);
  int GetViewPointCoveredFrontierPointNum(int viewpoint_ind,
                                          bool use_array_ind = false);
  int GetViewPointCoveredPointNum(const std::vector<bool> &point_list,
                                  int viewpoint_index,
                                  bool use_array_ind = false);
  int GetViewPointCoveredFrontierPointNum(
      const std::vector<bool> &frontier_point_list, int viewpoint_index,
      bool use_array_ind = false);
  void UpdateViewPointCoveredPoint(std::vector<bool> &point_list,
                                   int viewpoint_index,
                                   bool use_array_ind = false);
  void
  UpdateViewPointCoveredFrontierPoint(std::vector<bool> &frontier_point_list,
                                      int viewpoint_index,
                                      bool use_array_ind = false);
  ...
  nav_msgs::Path
  GetViewPointShortestPath(const Eigen::Vector3d &start_position,
                           const Eigen::Vector3d &target_position);
  bool
  GetViewPointShortestPathWithMaxLength(const Eigen::Vector3d &start_position,
                                        const Eigen::Vector3d &target_position,
  ...
  bool UseFrontier() { return vp_.kUseFrontier; }
  // For visualization
  void GetVisualizationCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &vis_cloud);
  void
  GetCollisionViewPointVisCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud);
