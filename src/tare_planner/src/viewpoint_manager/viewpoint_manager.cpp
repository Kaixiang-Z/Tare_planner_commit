...
bool ViewPointManagerParameter::ReadParameters(ros::NodeHandle &nh) {
  ...
  kNumber.x() =
      misc_utils_ns::getParam<int>(nh, "viewpoint_manager/number_x", 80);
  kNumber.y() =
      misc_utils_ns::getParam<int>(nh, "viewpoint_manager/number_y", 80);
  kNumber.z() =
      misc_utils_ns::getParam<int>(nh, "viewpoint_manager/number_z", 40);
  kViewPointNumber = kNumber.x() * kNumber.y() * kNumber.z();
  kRolloverStepsize = kNumber / 5;

  kResolution.x() = misc_utils_ns::getParam<double>(
      nh, "viewpoint_manager/resolution_x", 0.5);
  kResolution.y() = misc_utils_ns::getParam<double>(
      nh, "viewpoint_manager/resolution_y", 0.5);
  kResolution.z() = misc_utils_ns::getParam<double>(
      nh, "viewpoint_manager/resolution_z", 0.5);
  ...
  kViewPointCollisionMarginZPlus = misc_utils_ns::getParam<double>(
      nh, "kViewPointCollisionMarginZPlus", 0.5);
  kViewPointCollisionMarginZMinus = misc_utils_ns::getParam<double>(
      nh, "kViewPointCollisionMarginZMinus", 0.5);
  kCollisionGridZScale =
      misc_utils_ns::getParam<double>(nh, "kCollisionGridZScale", 2.0);
  ...
  kLineOfSightStopAtNearestObstacle = misc_utils_ns::getParam<bool>(
      nh, "kLineOfSightStopAtNearestObstacle", true);
  kCheckDynamicObstacleCollision =
      misc_utils_ns::getParam<bool>(nh, "kCheckDynamicObstacleCollision", true);
  kCollisionFrameCountMax =
      misc_utils_ns::getParam<int>(nh, "kCollisionFrameCountMax", 3);
  kViewPointHeightFromTerrain =
      misc_utils_ns::getParam<double>(nh, "kViewPointHeightFromTerrain", 0.75);
  kViewPointHeightFromTerrainChangeThreshold = misc_utils_ns::getParam<double>(
      nh, "kViewPointHeightFromTerrainChangeThreshold", 0.6);

  kCollisionPointThr =
      misc_utils_ns::getParam<int>(nh, "kCollisionPointThr", 3);
  ...
  kCoverageOcclusionThr =
      misc_utils_ns::getParam<double>(nh, "kCoverageOcclusionThr", 1.0);
  ...
  kInFovXYDistThreshold =
      3 * (kCoveragePointCloudResolution / 2) / tan(M_PI / 15);
...
ViewPointManager::ViewPointManager(ros::NodeHandle &nh) : initialized_(false) {
  ...
  kdtree_viewpoint_candidate_ = pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr(
      new pcl::KdTreeFLANN<pcl::PointXYZI>());
  kdtree_viewpoint_in_collision_ = pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr(
      new pcl::KdTreeFLANN<pcl::PointXYZI>());
      ...
      for (int z = 0; z < vp_.kNumber.z(); z++) {
        ...
        for (int i = 0; i < idx_addon.size(); i++) {
          ...
          if (grid_->InRange(neighbor_sub)) {
            ...
            double dist = sqrt(vp_.kResolution.x() * vp_.kResolution.x() *
                                   std::abs(idx_addon[i].x()) +
                               vp_.kResolution.y() * vp_.kResolution.y() *
                                   std::abs(idx_addon[i].y()) +
                               vp_.kResolution.z() * vp_.kResolution.z() *
                                   std::abs(idx_addon[i].z()));
...
void ViewPointManager::ComputeInRangeNeighborIndices() {
  ...
  for (int i = 0; i < in_range_neighbor_indices_.size(); i++) {
    ...
    kdtree->radiusSearch(point, vp_.kNeighborRange, in_range_indices,
                         in_range_sqdist);
...
void ViewPointManager::GetCollisionCorrespondence() {
  ...
  // std::cout << "computing collision grid viewpoint cloud size: " <<
  // viewpoint_cloud->points.size() << std::endl;
  ...
  for (int x = 0; x < vp_.kCollisionGridSize.x(); x++) {
    for (int y = 0; y < vp_.kCollisionGridSize.y(); y++) {
      for (int z = 0; z < vp_.kCollisionGridSize.z(); z++) {
        Eigen::Vector3d query_point_position =
            collision_grid_->Sub2Pos(x, y, z);
...
bool ViewPointManager::UpdateRobotPosition(
  ...
  for (int i = 0; i < vp_.dimension_; i++) {
    robot_grid_sub(i) =
        diff(i) > 0 ? static_cast<int>(diff(i) / (vp_.kRolloverStepsize(i) *
                                                  vp_.kResolution(i)))
                    : -1;
  }
  ...
  for (int i = 0; i < vp_.dimension_; i++) {
    sub_diff(i) =
        (vp_.kNumber(i) / vp_.kRolloverStepsize(i)) / 2 - robot_grid_sub(i);
  }
  ...
  rollover_step.x() = std::abs(sub_diff.x()) > 0
                          ? vp_.kRolloverStepsize.x() *
                                ((sub_diff.x() > 0) ? 1 : -1) *
                                std::abs(sub_diff.x())
                          : 0;
  rollover_step.y() = std::abs(sub_diff.y()) > 0
                          ? vp_.kRolloverStepsize.y() *
                                ((sub_diff.y() > 0) ? 1 : -1) *
                                std::abs(sub_diff.y())
                          : 0;
  rollover_step.z() = std::abs(sub_diff.z()) > 0
                          ? vp_.kRolloverStepsize.z() *
                                ((sub_diff.z() > 0) ? 1 : -1) *
                                std::abs(sub_diff.z())
                          : 0;

  // std::cout << "rolling x: " << rollover_step.x() << " y: " <<
  // rollover_step.y() << " z: " << rollover_step.z()
  //           << std::endl;
...
void ViewPointManager::UpdateOrigin() {
  for (int i = 0; i < vp_.dimension_; i++) {
    origin_(i) =
        robot_position_(i) - (vp_.kResolution(i) * vp_.kNumber(i)) / 2.0;
  }
}

int ViewPointManager::GetViewPointArrayInd(int viewpoint_ind,
...
void ViewPointManager::GetVisualizationCloud(
    pcl::PointCloud<pcl::PointXYZI>::Ptr &vis_cloud) {
  vis_cloud->clear();
  for (int i = 0; i < vp_.kViewPointNumber; i++) {
...
void ViewPointManager::CheckViewPointCollisionWithCollisionGrid(
  ...
  collision_grid_origin_ =
      origin_ - Eigen::Vector3d::Ones() * vp_.kViewPointCollisionMargin;
  ...
  for (const auto &point : collision_cloud->points) {
    ...
    if (collision_grid_->InRange(collision_grid_sub)) {
      ...
      if (collision_point_count_[collision_grid_ind] >=
          vp_.kCollisionPointThr) {
...
bool ViewPointManager::InCurrentFrameLineOfSight(
...
void ViewPointManager::CheckViewPointBoundaryCollision() {
  ...
  for (int i = 0; i < vp_.kViewPointNumber; i++) {
    ...
    if ((!viewpoint_boundary_.points.empty() &&
         !misc_utils_ns::PointInPolygon(viewpoint_position,
                                        viewpoint_boundary_))) {
      SetViewPointCollision(i, true, true);
      continue;
    }
    for (int j = 0; j < nogo_boundary_.size(); j++) {
      if (!nogo_boundary_[j].points.empty() &&
          misc_utils_ns::PointInPolygon(viewpoint_position,
                                        nogo_boundary_[j])) {
        SetViewPointCollision(i, true, true);
...
void ViewPointManager::CheckViewPointCollisionWithTerrain(
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr collision_cloud(
      new pcl::PointCloud<pcl::PointXYZI>());
...
void ViewPointManager::CheckViewPointLineOfSightHelper(
    const Eigen::Vector3i &start_sub, const Eigen::Vector3i &end_sub,
  ...
  if (ray_cast_cells.size() > 1) {
    if (vp_.kLineOfSightStopAtNearestObstacle) {
      ...
      for (int i = 1; i < ray_cast_cells.size(); i++) {
        ...
        if (ViewPointInCollision(viewpoint_ind) &&
            GetViewPointCollisionFrameCount(viewpoint_ind) == 0) {
          occlude = true;
          break;
        }
        if (!occlude) {
          ...
          if (vp_.kCheckDynamicObstacleCollision &&
              GetViewPointCollisionFrameCount(viewpoint_ind) >
                  vp_.kCollisionFrameCountMax)
...
void ViewPointManager::CheckViewPointLineOfSight() {
  ...
  std::vector<Eigen::Vector3i> ray_cast_cells;
  Eigen::Vector3i max_sub(vp_.kNumber.x() - 1, vp_.kNumber.y() - 1,
                          vp_.kNumber.z() - 1);
...
bool ViewPointManager::InFOVAndRange(
...
void ViewPointManager::CheckViewPointConnectivity() {
  ...
  if (ViewPointInCollision(robot_ind)) {
    // std::cout << "ViewPointManager::CheckViewPointConnectivity: robot in
    // collision" << std::endl; return; Find the nearest viewpoint that is not
    // in collision
  ...
  while (!queue.empty()) {
    ...
    for (int i = 0; i < connected_neighbor_indices_[cur_ind].size(); i++) {
      ...
      if (!grid_->InRange(neighbor_ind)) {
        std::cout << "ViewPointManager::CheckViewPointConnectivity: neighbor "
                     "ind out of bound"
                  << std::endl;
        continue;
      }
      if (!checked[neighbor_ind] && !ViewPointInCollision(neighbor_ind) &&
        ...
        if (std::abs(GetViewPointHeight(cur_ind) -
                     GetViewPointHeight(neighbor_ind)) <
...
void ViewPointManager::UpdateViewPointVisited(
  ...
  for (int i = 0; i < viewpoints_.size(); i++) {
    geometry_msgs::Point viewpoint_position = GetViewPointPosition(i, true);
    int cell_ind = grid_world->GetCellInd(
        viewpoint_position.x, viewpoint_position.y, viewpoint_position.z);
    if (grid_world->IndInBound((cell_ind))) {
      grid_world_ns::CellStatus cell_status =
          grid_world->GetCellStatus(cell_ind);
...
void ViewPointManager::SetViewPointHeightWithTerrain(
  ...
  // Set the height of the viewpoint nearby the robot to be the height of the
  // robot, in case there is no terrain cloud within the blind spot.
  if (!initialized_)
    return;
  Eigen::Vector3i robot_sub = GetViewPointSub(robot_position_);
  int robot_ind = grid_->Sub2Ind(robot_sub);
  MY_ASSERT(grid_->InRange(robot_sub));
  geometry_msgs::Point robot_viewpoint_position =
      GetViewPointPosition(robot_ind);
  ...
  if (!ViewPointHasTerrainHeight(robot_ind) ||
    ...
    for (int i = 0; i < in_range_neighbor_indices_[robot_ind].size(); i++) {
      ...
      if (!ViewPointHasTerrainHeight(neighbor_ind) ||
          std::abs(GetViewPointHeight(neighbor_ind) - robot_position_.z()) >
              0.6) {
  ...
  for (const auto &terrain_point : terrain_cloud->points) {
    ...
    Eigen::Vector3i viewpoint_sub = GetViewPointSub(
        Eigen::Vector3d(terrain_point.x, terrain_point.y, terrain_point.z));
    if (grid_->InRange(viewpoint_sub)) {
      int viewpoint_ind = grid_->Sub2Ind(viewpoint_sub);
      double target_height = terrain_point.z + vp_.kViewPointHeightFromTerrain;
      // If the viewpoint has not been set height with terrain points, or if
      // there is a terrain point with a lower height
  ...
  // For viewpoints that are not set heights with terrain directly, use
  // neighbors' heights
  for (int i = 0; i < vp_.kViewPointNumber; i++) {
...
// Collision
bool ViewPointManager::ViewPointInCollision(int viewpoint_ind,
                                            bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].InCollision();
}
void ViewPointManager::SetViewPointCollision(int viewpoint_ind,
                                             bool in_collision,
                                             bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
...
// Line of Sight
bool ViewPointManager::ViewPointInLineOfSight(int viewpoint_ind,
                                              bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].InLineOfSight();
}
void ViewPointManager::SetViewPointInLineOfSight(int viewpoint_ind,
...
// Connectivity
bool ViewPointManager::ViewPointConnected(int viewpoint_ind,
                                          bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].Connected();
}
void ViewPointManager::SetViewPointConnected(int viewpoint_ind, bool connected,
                                             bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
...
void ViewPointManager::SetViewPointVisited(int viewpoint_ind, bool visited,
                                           bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetVisited(visited);
}
// Selected
bool ViewPointManager::ViewPointSelected(int viewpoint_ind,
                                         bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].Selected();
}
void ViewPointManager::SetViewPointSelected(int viewpoint_ind, bool selected,
                                            bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetSelected(selected);
}
// Candidacy
bool ViewPointManager::IsViewPointCandidate(int viewpoint_ind,
                                            bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].IsCandidate();
}
void ViewPointManager::SetViewPointCandidate(int viewpoint_ind, bool candidate,
                                             bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetCandidate(candidate);
}
// Terrain Height
bool ViewPointManager::ViewPointHasTerrainHeight(int viewpoint_ind,
                                                 bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].HasTerrainHeight();
}
void ViewPointManager::SetViewPointHasTerrainHeight(int viewpoint_ind,
                                                    bool has_terrain_height,
                                                    bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetHasTerrainHeight(has_terrain_height);
}
// In exploring cell
bool ViewPointManager::ViewPointInExploringCell(int viewpoint_ind,
                                                bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].InExploringCell();
}
void ViewPointManager::SetViewPointInExploringCell(int viewpoint_ind,
                                                   bool in_exploring_cell,
                                                   bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetInExploringCell(in_exploring_cell);
}
// Height
double ViewPointManager::GetViewPointHeight(int viewpoint_ind,
                                            bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetHeight();
}
void ViewPointManager::SetViewPointHeight(int viewpoint_ind, double height,
                                          bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetHeight(height);
}
// In current frame line of sight
bool ViewPointManager::ViewPointInCurrentFrameLineOfSight(int viewpoint_ind,
                                                          bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].InCurrentFrameLineOfSight();
}
void ViewPointManager::SetViewPointInCurrentFrameLineOfSight(
    int viewpoint_ind, bool in_current_frame_line_of_sight,
    bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetInCurrentFrameLineOfSight(
      in_current_frame_line_of_sight);
}
// Position
geometry_msgs::Point
ViewPointManager::GetViewPointPosition(int viewpoint_ind, bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetPosition();
}
void ViewPointManager::SetViewPointPosition(int viewpoint_ind,
                                            geometry_msgs::Point position,
                                            bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
...
// Cell Ind
int ViewPointManager::GetViewPointCellInd(int viewpoint_ind,
                                          bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetCellInd();
}
void ViewPointManager::SetViewPointCellInd(int viewpoint_ind, int cell_ind,
                                           bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetCellInd(cell_ind);
}
// Collision frame count
int ViewPointManager::GetViewPointCollisionFrameCount(int viewpoint_ind,
                                                      bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetCollisionFrameCount();
}
void ViewPointManager::AddViewPointCollisionFrameCount(int viewpoint_ind,
                                                       bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].AddCollisionFrame();
}
void ViewPointManager::ResetViewPointCollisionFrameCount(int viewpoint_ind,
                                                         bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].ResetCollisionFrameCount();
}
// Covered point list
void ViewPointManager::ResetViewPointCoveredPointList(int viewpoint_ind,
                                                      bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].ResetCoveredPointList();
  viewpoints_[array_ind].ResetCoveredFrontierPointList();
}
void ViewPointManager::AddUncoveredPoint(int viewpoint_ind, int point_ind,
                                         bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].AddCoveredPoint(point_ind);
}
void ViewPointManager::AddUncoveredFrontierPoint(int viewpoint_ind,
                                                 int point_ind,
                                                 bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
...
ViewPointManager::GetViewPointCoveredPointList(int viewpoint_ind,
                                               bool use_array_ind) const {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
...
const std::vector<int> &ViewPointManager::GetViewPointCoveredFrontierPointList(
    int viewpoint_ind, bool use_array_ind) const {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetCoveredFrontierPointList();
}

int ViewPointManager::GetViewPointCoveredPointNum(int viewpoint_ind,
                                                  bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetCoveredPointNum();
}

int ViewPointManager::GetViewPointCoveredFrontierPointNum(int viewpoint_ind,
                                                          bool use_array_ind) {
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].GetCoveredFrontierPointNum();
}

int ViewPointManager::GetViewPointCoveredPointNum(
    const std::vector<bool> &point_list, int viewpoint_index,
    bool use_array_ind) {
  int covered_point_num = 0;
  for (const auto &point_ind :
       GetViewPointCoveredPointList(viewpoint_index, use_array_ind)) {
...
int ViewPointManager::GetViewPointCoveredFrontierPointNum(
    const std::vector<bool> &frontier_point_list, int viewpoint_index,
    bool use_array_ind) {
...
void ViewPointManager::UpdateViewPointCoveredPoint(
...
int ViewPointManager::GetViewPointCandidate() {
  ...
  for (int i = 0; i < vp_.kViewPointNumber; i++) {
    ...
    if (!ViewPointInCollision(i) && ViewPointInLineOfSight(i) &&
        ViewPointConnected(i)) {
  ...
  // std::cout << "candidate viewpoint num: " << candidate_indices_.size() <<
  // std::endl;
  ...
  if (!viewpoint_in_collision_cloud_->points.empty()) {
    kdtree_viewpoint_in_collision_->setInputCloud(
        viewpoint_in_collision_cloud_);
  }

  // Construct a graph of all the viewpoints
  GetCandidateViewPointGraph(candidate_viewpoint_graph_,
...
nav_msgs::Path
ViewPointManager::GetViewPointShortestPath(int start_viewpoint_ind,
                                           int target_viewpoint_ind) {
  nav_msgs::Path path;
  if (!InRange(start_viewpoint_ind)) {
    ROS_WARN_STREAM(
        "ViewPointManager::GetViewPointShortestPath start viewpoint ind: "
        << start_viewpoint_ind << " not in range");
    return path;
  }
  if (!InRange(target_viewpoint_ind)) {
    ROS_WARN_STREAM(
        "ViewPointManager::GetViewPointShortestPath target viewpoint ind: "
  ...
  double path_length = misc_utils_ns::AStarSearch(
      candidate_viewpoint_graph_, candidate_viewpoint_dist_,
      candidate_viewpoint_position_, start_graph_ind, target_graph_ind, true,
      path_graph_indices);
...
bool ViewPointManager::GetViewPointShortestPathWithMaxLength(
  ...
  bool found_path = misc_utils_ns::AStarSearchWithMaxPathLength(
      candidate_viewpoint_graph_, candidate_viewpoint_dist_,
      candidate_viewpoint_position_, start_graph_ind, target_graph_ind, true,
      path_graph_indices, shortest_path_dist, max_path_length);
...
void ViewPointManager::UpdateCandidateViewPointCellStatus(
  ...
  for (const auto &ind : candidate_indices_) {
    ...
    if (grid_world->IndInBound(cell_ind)) {
      ...
      ROS_WARN_STREAM(
          "ViewPointManager::UpdateCandidateViewPointCellStatus: cell ind "
...
int ViewPointManager::GetNearestCandidateViewPointInd(
    const Eigen::Vector3d &position) {
  int viewpoint_ind = GetViewPointInd(position);
  if (InRange(viewpoint_ind)) {
  ...
  if (!candidate_indices_.empty()) {
    ...
    for (const auto &cur_viewpoint_ind : candidate_indices_) {
      geometry_msgs::Point cur_position =
          GetViewPointPosition(cur_viewpoint_ind);
    ...
    std::cout << "Candidate viewpoint empty, can't find nearest candidate "
                 "viewpoints to the position"
              << std::endl;
    return -1;
  }
}
...
bool ViewPointManager::InLocalPlanningHorizon(const Eigen::Vector3d &position) {
  ...
  if (InRange(viewpoint_ind)) {
    ...
    if (std::abs(viewpoint_position.z - position.z()) < max_z_diff &&
        (IsViewPointCandidate(viewpoint_ind) ||
         ViewPointInCollision(viewpoint_ind))) {
      return true;
    }
  }
  return false;
