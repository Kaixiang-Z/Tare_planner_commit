...
PointCloudManager::PointCloudManager(int row_num, int col_num, int level_num,
                                     int max_cell_point_num, double cell_size,
                                     double cell_height, int neighbor_cell_num)
    : kRowNum(row_num), kColNum(col_num), kLevelNum(level_num),
      kMaxCellPointNum(max_cell_point_num), kCellSize(cell_size),
      kCellHeight(cell_height), kNeighborCellNum(neighbor_cell_num),
  ...
  pointcloud_grid_ = std::make_unique<grid_ns::Grid<PCLCloudTypePtr>>(
      pointcloud_grid_size, cloud_ptr_tmp, pointcloud_grid_origin,
  ...
  for (int i = 0; i < occupancy_cloud_grid_->GetCellNumber(); i++) {
    occupancy_cloud_grid_->GetCell(i) = pcl::PointCloud<pcl::PointXYZI>::Ptr(
        new pcl::PointCloud<pcl::PointXYZI>);
  }

  cloud_dwz_filter_.setLeafSize(kCloudDwzFilterLeafSize,
...
void PointCloudManager::UpdateOrigin() {
  ...
  occupancy_cloud_grid_->SetOrigin(
      Eigen::Vector3d(origin_.x, origin_.y, origin_.z));
}

bool PointCloudManager::UpdateRobotPosition(
  ...
  misc_utils_ns::SetDifference(neighbor_indices_, prev_neighbor_indices_,
                               indices_diff);
  bool rolling = false;
  ...
  Eigen::Vector3i neighbor_cell_min_sub =
      robot_cell_sub - Eigen::Vector3i(N, N, N);
  neighbor_cells_origin_ =
      pointcloud_grid_->Sub2Pos(neighbor_cell_min_sub) -
...
PointCloudManager::GetRolledInOccupancyCloud() {
...
void PointCloudManager::StoreOccupancyCloud(
    const pcl::PointCloud<pcl::PointXYZI>::Ptr &occupancy_cloud) {
  for (const auto &point : occupancy_cloud->points) {
    Eigen::Vector3i sub = occupancy_cloud_grid_->Pos2Sub(
...
void PointCloudManager::UpdateCoveredCloudPoints(int cloud_index,
                                                 int point_index) {
