...
PointCloudManager::PointCloudManager(int row_num, int col_num, int level_num, int max_cell_point_num, double cell_size,
    ...
    kRowNum(row_num),
    kColNum(col_num), kLevelNum(level_num), kMaxCellPointNum(max_cell_point_num), kCellSize(cell_size),
...
void PointCloudManager::ClearNeighborCellOccupancyCloud() {
    for (const auto &neighbor_ind : neighbor_indices_) { occupancy_cloud_grid_->GetCell(neighbor_ind)->clear(); }
}
...
void PointCloudManager::GetOccupancyCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &occupancy_cloud) {
    ...
    for (int i = 0; i < cell_num; i++) { *occupancy_cloud += *(occupancy_cloud_grid_->GetCell(i)); }
}
...
void PointCloudManager::StoreOccupancyCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr &occupancy_cloud) {
    for (const auto &point : occupancy_cloud->points) {
        ...
        if (!occupancy_cloud_grid_->InRange(sub)) { continue; }
...
int PointCloudManager::GetAllPointNum() {
    int num = 0;
    for (int i = 0; i < pointcloud_grid_->GetCellNumber(); ++i) { num += pointcloud_grid_->GetCell(i)->points.size(); }
    return num;
}
...
void PointCloudManager::UpdateOldCloudPoints() {
    for (int i = 0; i < pointcloud_grid_->GetCellNumber(); ++i) {
        ...
        for (int j = 0; j < point_num; ++j) { pointcloud_grid_->GetCell(i)->points[j].r = 255; }
    }
}
...
void PointCloudManager::UpdateCoveredCloudPoints() {
    for (int i = 0; i < pointcloud_grid_->GetCellNumber(); ++i) {
        ...
        for (int j = 0; j < point_num; ++j) {
            if (pointcloud_grid_->GetCell(i)->points[j].g > 0) { pointcloud_grid_->GetCell(i)->points[j].g = 255; }
        }
    }
}
