...
KeyposeGraph::KeyposeGraph(ros::NodeHandle &nh) :
    allow_vertical_edge_(false), current_keypose_id_(0), kAddNodeMinDist(1.0), kAddEdgeCollisionCheckResolution(0.4),
    kAddEdgeCollisionCheckRadius(0.3), kAddEdgeConnectDistThr(3.0), kAddEdgeToLastKeyposeDistThr(3.0),
...
void KeyposeGraph::GetMarker(visualization_msgs::Marker &node_marker, visualization_msgs::Marker &edge_marker) {
    ...
    for (const auto &node : nodes_) { node_marker.points.push_back(node.position_); }
...
void KeyposeGraph::GetConnectedNodeIndices(int query_ind, std::vector<int> &connected_node_indices,
    ...
    while (!dfs_stack.empty()) {
        ...
        for (int i = 0; i < graph_[current_ind].size(); i++) {
            ...
            if (!visited[neighbor_ind] && constraints[neighbor_ind]) { dfs_stack.push(neighbor_ind); }
        }
    }
}
...
void KeyposeGraph::CheckConnectivity(const geometry_msgs::Point &robot_position) {
    ...
    for (int i = 0; i < nodes_.size(); i++) { nodes_[i].is_connected_ = false; }
    ...
    if (!connected_nodes_cloud_->points.empty()) { kdtree_connected_nodes_->setInputCloud(connected_nodes_cloud_); }
}
                ...
                if (!in_range_node_indices.empty()) {
                    for (int idx = 0; idx < in_range_node_indices.size(); idx++) {
                        ...
                        if (in_range_ind >= 0 && in_range_ind < nodes_.size()) {
                            ...
                            if (!in_collision) { AddEdge(new_node_ind, in_range_ind, neighbor_node_dist); }
                        }
                    }
                }
                return new_node_ind;
...
bool KeyposeGraph::GetShortestPathWithMaxLength(const geometry_msgs::Point &start_point,
    ...
    for (int i = 0; i < nodes_.size(); i++) { node_positions.push_back(nodes_[i].position_); }
