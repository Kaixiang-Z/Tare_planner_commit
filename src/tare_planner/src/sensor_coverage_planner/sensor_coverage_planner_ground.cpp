...
bool PlannerParameters::ReadParameters(ros::NodeHandle &nh) {
    ...
    sub_reset_waypoint_topic_ =
        misc_utils_ns::getParam<std::string>(nh, "sub_reset_waypoint_topic_", "/reset_waypoint");
...
SensorCoveragePlanner3D::SensorCoveragePlanner3D(ros::NodeHandle &nh, ros::NodeHandle &nh_p) :
    ...
    lookahead_point_in_line_of_sight_(true), reset_waypoint_(false), registered_cloud_count_(0), keypose_count_(0),
...
bool SensorCoveragePlanner3D::initialize(ros::NodeHandle &nh, ros::NodeHandle &nh_p) {
    ...
    joystick_sub_ = nh.subscribe(pp_.sub_joystick_topic_, 1, &SensorCoveragePlanner3D::JoystickCallback, this);
    reset_waypoint_sub_ =
...
void SensorCoveragePlanner3D::NogoBoundaryCallback(const geometry_msgs::PolygonStampedConstPtr &polygon_msg) {
    if (polygon_msg->polygon.points.empty()) { return; }
...
void SensorCoveragePlanner3D::UpdateGlobalRepresentation() {
    ...
    if (exploration_finished_ && pp_.kNoExplorationReturnHome) { pd_.planning_env_->SetUseFrontier(false); }
    ...
    if (!pd_.grid_world_->HomeSet()) { pd_.grid_world_->SetHomePosition(pd_.initial_position_); }
}
...
void SensorCoveragePlanner3D::LocalPlanning(int uncovered_point_num, int uncovered_frontier_point_num,
    ...
    if (lookahead_point_update_) { pd_.local_coverage_planner_->SetLookAheadPoint(pd_.lookahead_point_); }
...
bool SensorCoveragePlanner3D::GetLookAheadPoint(const exploration_path_ns::ExplorationPath &local_path,
    ...
    for (int i = 1; i < global_path.nodes_.size(); i++) {
        ...
        if (global_path.nodes_[i].type_ == exploration_path_ns::NodeType::GLOBAL_VIEWPOINT) { break; }
    }
    ...
    if (reset_waypoint_) { has_lookahead = false; }
    ...
    for (int i = robot_i + 1; i < local_path.GetNodeNum(); i++) {
        if (local_path.nodes_[i].type_ == exploration_path_ns::NodeType::LOCAL_VIEWPOINT) { forward_viewpoint_count++; }
    }
    ...
    for (int i = robot_i + 1; i < local_path.GetNodeNum(); i++) {
        ...
        {
            if (pp_.kUseLineOfSightLookAheadPoint && !in_line_of_sight) { forward_lookahead_point_in_los = false; }
    ...
    for (int i = robot_i - 1; i >= 0; i--) {
        ...
        {
            if (pp_.kUseLineOfSightLookAheadPoint && !in_line_of_sight) { backward_lookahead_point_in_los = false; }
...
void SensorCoveragePlanner3D::PublishRuntime() {
    ...
    if (!exploration_finished_ && pp_.kNoExplorationReturnHome) {
        for (int i = 0; i < runtime_breakdown_msg.data.size() - 1; i++) { runtime += runtime_breakdown_msg.data[i]; }
    }
    ...
    if (current_moving_direction_.norm() > 0.5) {
        ...
        if (pd_.moving_direction_.dot(current_moving_direction_) < 0) {
            ...
            if (direction_change_count_ > pp_.kDirectionChangeCounterThr) {
                if (!use_momentum_) { momentum_activation_count_++; }
                use_momentum_ = true;
            }
        } else {
...
void SensorCoveragePlanner3D::execute(const ros::TimerEvent &) {
    ...
    if (keypose_cloud_update_) {
        ...
        if (pd_.grid_world_->IsReturningHome() && pd_.local_coverage_planner_->IsLocalCoverageComplete()
            ...
            if (!exploration_finished_) { PrintExplorationStatus("Exploration completed, returning home", false); }
