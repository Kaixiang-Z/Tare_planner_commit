...
Cell::Cell(double x, double y, double z)
    : in_horizon_(false), robot_position_set_(false), visit_count_(0),
      keypose_id_(0), path_added_to_keypose_graph_(false),
...
Cell::Cell(const geometry_msgs::Point &center)
    : Cell(center.x, center.y, center.z) {}
...
bool Cell::IsCellConnected(int cell_ind) {
  if (std::find(connected_cell_indices_.begin(), connected_cell_indices_.end(),
                cell_ind) != connected_cell_indices_.end()) {
    return true;
  } else {
    return false;
  }
...
GridWorld::GridWorld(ros::NodeHandle &nh)
    : initialized_(false), use_keypose_graph_(false) {
  ReadParameters(nh);
  ...
  Cell cell_tmp;
  subspaces_ = std::make_unique<grid_ns::Grid<Cell>>(
      grid_size, cell_tmp, grid_origin, grid_resolution);
...
GridWorld::GridWorld(int row_num, int col_num, int level_num, double cell_size,
                     double cell_height, int nearby_grid_num)
    : kRowNum(row_num), kColNum(col_num), kLevelNum(level_num),
      kCellSize(cell_size), kCellHeight(cell_height),
      KNearbyGridNum(nearby_grid_num), kMinAddPointNumSmall(60),
      kMinAddPointNumBig(100), kMinAddFrontierPointNum(30),
      kCellExploringToCoveredThr(1), kCellCoveredToExploringThr(10),
      kCellExploringToAlmostCoveredThr(10),
      kCellAlmostCoveredToExploringThr(20), kCellUnknownToExploringThr(1),
      cur_keypose_id_(0), cur_keypose_graph_node_ind_(0),
      cur_robot_cell_ind_(-1), prev_robot_cell_ind_(-1), cur_keypose_(0, 0, 0),
      initialized_(false), use_keypose_graph_(false) {
  robot_position_.x = 0.0;
  robot_position_.y = 0.0;
  robot_position_.z = 0.0;
...
void GridWorld::ReadParameters(ros::NodeHandle &nh) {
  ...
  int viewpoint_number =
      misc_utils_ns::getParam<int>(nh, "viewpoint_manager/number_x", 40);
  double viewpoint_resolution = misc_utils_ns::getParam<double>(
      nh, "viewpoint_manager/resolution_x", 1.0);
  kCellSize = viewpoint_number * viewpoint_resolution / 5;
  kCellHeight =
      misc_utils_ns::getParam<double>(nh, "kGridWorldCellHeight", 8.0);
  KNearbyGridNum =
      misc_utils_ns::getParam<int>(nh, "kGridWorldNearbyGridNum", 5);
  kMinAddPointNumSmall =
      misc_utils_ns::getParam<int>(nh, "kMinAddPointNumSmall", 60);
  kMinAddPointNumBig =
      misc_utils_ns::getParam<int>(nh, "kMinAddPointNumBig", 100);
  kMinAddFrontierPointNum =
      misc_utils_ns::getParam<int>(nh, "kMinAddFrontierPointNum", 30);
...
void GridWorld::UpdateNeighborCells(
    ...
    for (int i = 0; i < kRowNum; i++) {
      for (int j = 0; j < kColNum; j++) {
        for (int k = 0; k < kLevelNum; k++) {
          Eigen::Vector3d subspace_center_position =
              subspaces_->Sub2Pos(i, j, k);
          ...
          subspaces_->GetCell(i, j, k).SetPosition(
              subspace_center_geo_position);
  ...
  for (const auto &cell_ind : neighbor_cell_indices_) {
    if (std::find(prev_neighbor_cell_indices.begin(),
                  prev_neighbor_cell_indices.end(),
                  cell_ind) == prev_neighbor_cell_indices.end()) {
...
void GridWorld::UpdateRobotPosition(
    const geometry_msgs::Point &robot_position) {
...
void GridWorld::UpdateCellKeyposeGraphNodes(
  ...
  for (const auto &node_ind : keypose_graph_connected_node_indices) {
    ...
    int cell_ind =
        GetCellInd(node_position.x, node_position.y, node_position.z);
...
void GridWorld::GetCellSub(int &row_idx, int &col_idx, int &level_idx,
  ...
  for (int i = 0; i < subspaces_->GetCellNumber(); i++) {
    ...
    if (!subspaces_->GetCell(i).GetConnectedCellIndices().empty()) {
      ...
      Eigen::Vector3d position =
          subspaces_->GetCell(i).GetRoadmapConnectionPoint();
      point.x = position.x();
      point.y = position.y();
      point.z = position.z();
      point.intensity = i;
...
void GridWorld::GetNeighborCellIndices(const geometry_msgs::Point &position,
                                       const Eigen::Vector3i &neighbor_range,
                                       std::vector<int> &neighbor_indices) {
  Eigen::Vector3i center_cell_sub =
...
void GridWorld::GetExploringCellIndices(
...
void GridWorld::SetCellRobotPosition(
    int cell_ind, const geometry_msgs::Point &robot_position) {
  MY_ASSERT(subspaces_->InRange(cell_ind));
...
void GridWorld::UpdateCellStatus(
  ...
  for (const auto &viewpoint_ind : viewpoint_manager->candidate_indices_) {
    ...
    Eigen::Vector3i sub = subspaces_->Pos2Sub(Eigen::Vector3d(
  ...
  for (const auto &cell_ind : neighbor_cell_indices_) {
    if (subspaces_->GetCell(cell_ind).GetStatus() ==
    ...
    // Exploring to Almost covered
      ...
      if (subspaces_->GetCell(cell_ind).GetVisitCount() == 1 &&
        ...
        if (xy_dist_to_robot < kCellSize &&
            z_dist_to_robot < kCellHeight * 0.8) {
          subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
        }
      }
    }
  ...
  for (const auto &cell_ind : almost_covered_cell_indices_) {
    if (std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(),
                  cell_ind) == neighbor_cell_indices_.end()) {
      subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
      almost_covered_cell_indices_.erase(
          std::remove(almost_covered_cell_indices_.begin(),
                      almost_covered_cell_indices_.end(), cell_ind),
...
exploration_path_ns::ExplorationPath GridWorld::SolveGlobalTSP(
  ...
  keypose_graph->GetClosestConnectedNodeIndAndDistance(
      robot_position_, closest_node_ind, closest_node_dist);
  if (closest_node_dist < kCellSize / 2 && closest_node_ind >= 0 &&
      closest_node_ind < keypose_graph->GetNodeNum()) {
    global_path_robot_position =
        keypose_graph->GetNodePosition(closest_node_ind);
  } else if (cur_keypose_graph_node_ind_ >= 0 &&
             cur_keypose_graph_node_ind_ < keypose_graph->GetNodeNum()) {
    // ROS_WARN("GridWorld::SolveGlobalTSP: using nearest keypose node for robot
    // position");
    global_path_robot_position =
        keypose_graph->GetNodePosition(cur_keypose_graph_node_ind_);
  } else {
    // ROS_WARN("GridWorld::SolveGlobalTSP: using neighbor cell roadmap
  ...
  for (int i = 0; i < subspaces_->GetCellNumber(); i++) {
    if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING) {
      if (std::find(neighbor_cell_indices_.begin(),
                    neighbor_cell_indices_.end(),
                    i) == neighbor_cell_indices_.end() ||
        ...
        if (!use_keypose_graph_ || keypose_graph == nullptr ||
          ...
          if (keypose_graph->IsPositionReachable(connection_point_geo)) {
            ...
            // Check all the keypose graph nodes within this cell to see if
            // there are any connected nodes
  ...
  if (exploring_cell_indices.empty()) {
    ...
    if (!use_keypose_graph_ || keypose_graph == nullptr ||
      ...
      keypose_graph->GetShortestPath(global_path_robot_position, home_position,
                                     true, return_home_path, false);
    ...
    for (int j = 0; j < i; j++) {
      if (!use_keypose_graph_ || keypose_graph == nullptr ||
        ...
        distance_matrix[i][j] = static_cast<int>(
            10 * misc_utils_ns::PointXYZDist<geometry_msgs::Point,
                                             geometry_msgs::Point>(
        ...
        distance_matrix[i][j] = static_cast<int>(
            10 * keypose_graph->GetShortestPath(exploring_cell_positions[i],
                                                exploring_cell_positions[j],
                                                false, path_tmp, false));
      }
    }
  }
  ...
  if (!use_keypose_graph_ || keypose_graph == nullptr ||
    ...
    for (int i = 0; i < node_index.size(); i++) {
      ...
      exploration_path_ns::Node node(
          exploring_cell_positions[cell_ind],
    ...
    for (int i = 0; i < node_index.size() - 1; i++) {
      ...
      keypose_graph->GetShortestPath(cur_position, next_position, true,
                                     keypose_path, false);
...
void GridWorld::GetCellViewPointPositions(
    std::vector<Eigen::Vector3d> &viewpoint_positions) {
  viewpoint_positions.clear();
  for (int i = 0; i < subspaces_->GetCellNumber(); i++) {
    if (subspaces_->GetCell(i).GetStatus() !=
        grid_world_ns::CellStatus::EXPLORING) {
      continue;
    }
    if (std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(),
                  i) == neighbor_cell_indices_.end()) {
      viewpoint_positions.push_back(
          subspaces_->GetCell(i).GetViewPointPosition());
    }
  }
}
...
void GridWorld::AddPathsInBetweenCells(
  ...
  for (int i = 0; i < neighbor_cell_indices_.size(); i++) {
    ...
    if (!candidate_viewpoint_indices.empty()) {
      ...
      subspaces_->GetCell(cell_ind).SetRoadmapConnectionPoint(Eigen::Vector3d(
          min_dist_viewpoint_position.x, min_dist_viewpoint_position.y,
  ...
  for (int i = 0; i < neighbor_cell_indices_.size(); i++) {
    ...
    int viewpoint_num =
        subspaces_->GetCell(from_cell_ind).GetViewPointIndices().size();
    if (viewpoint_num == 0) {
      continue;
    }
        ...
        for (int z = -1; z <= 1; z++) {
          if (std::abs(x) + std::abs(y) + std::abs(z) == 1) {
            Eigen::Vector3i neighbor_sub =
                from_cell_sub + Eigen::Vector3i(x, y, z);
    ...
    for (int j = 0; j < nearby_cell_indices.size(); j++) {
      ...
      // TODO: change to: if there is already a direct keypose graph connection
      // then continue
      ...
      nav_msgs::Path path_in_between =
          viewpoint_manager->GetViewPointShortestPath(
              from_cell_roadmap_connection_position,
              to_cell_roadmap_connection_position);
      ...
      if (PathValid(path_in_between, from_cell_ind, to_cell_ind)) {
        ...
        // Add the path
        // std::cout << "Adding path between " << from_cell_ind << " " <<
        // to_cell_ind << std::endl;
        ...
        if (!connected) {
          ...
          subspaces_->GetCell(from_cell_ind)
              .SetRoadmapConnectionPointSet(false);
...
bool GridWorld::PathValid(const nav_msgs::Path &path, int from_cell_ind,
                          int to_cell_ind) {
  if (path.poses.size() >= 2) {
    for (const auto &pose : path.poses) {
      int cell_ind = GetCellInd(pose.pose.position.x, pose.pose.position.y,
...
bool GridWorld::HasDirectKeyposeGraphConnection(
  ...
  if (!keypose_graph->HasNode(start_position) ||
      !keypose_graph->HasNode(goal_position)) {
    return false;
  }

  // Search a path connecting start_position and goal_position with a max path
  // length constraint
  ...
  nav_msgs::Path path;
  bool found_path = keypose_graph->GetShortestPathWithMaxLength(
