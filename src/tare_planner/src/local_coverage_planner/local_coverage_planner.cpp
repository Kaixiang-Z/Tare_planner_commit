        ...
        if (viewpoint_manager_->InLocalPlanningHorizon(global_path.nodes_.front().position_)) {
            for (int i = 0; i < global_path.nodes_.size(); i++) {
                ...
                boundary_viewpoint_index =
                    viewpoint_manager_->GetNearestCandidateViewPointInd(global_path.nodes_[i].position_);
            }
        }
    }
...
void LocalCoveragePlanner::EnqueueViewpointCandidates(std::vector<std::pair<int, int>> &cover_point_queue,
    ...
    // Sort the queue， 根据点的数量排序
    std::sort(cover_point_queue.begin(), cover_point_queue.end(), SortPairInRev);
    if (use_frontier_) { std::sort(frontier_queue.begin(), frontier_queue.end(), SortPairInRev); }
}
...
void LocalCoveragePlanner::SelectViewPoint(const std::vector<std::pair<int, int>> &queue,
    ...
    for (int i = 0; i < covered.size(); i++) { covered_copy.push_back(covered[i]); }
    std::vector<std::pair<int, int>> queue_copy;
    for (int i = 0; i < queue.size(); i++) { queue_copy.push_back(queue[i]); }
    // 计算有多少个视点满足需求
    int sample_range = 0;
    for (int i = 0; i < queue_copy.size(); i++) {
        if (use_frontier) {
            if (queue_copy[i].first >= parameters_.kMinAddFrontierPointNum) { sample_range++; }
        } else {
            if (queue_copy[i].first >= parameters_.kMinAddPointNum) { sample_range++; }
        }
    }
    ...
    while (true) {
        ...
        // 对于每一个覆盖边界的点集， 将covered_copy对应的索引改为true
        if (use_frontier) {
            for (const auto &point_ind :
        ...
        // 该索引纳入被选择的视点集合，并擦除掉queue_copy内的对应数据
        ...
        if (use_frontier) {
            if (queue_copy.empty() || queue_copy[0].first < parameters_.kMinAddFrontierPointNum) { break; }
        }
        ...
        for (int i = 0; i < queue.size(); i++) {
            if (use_frontier) {
                if (queue[i].first >= parameters_.kMinAddFrontierPointNum) { sample_range++; }
            } else {
                if (queue[i].first >= parameters_.kMinAddPointNum) { sample_range++; }
            }
        }
        // 再随机选下一个点
        sample_range = std::min(parameters_.kGreedyViewPointSampleRange, sample_range);
...
LocalCoveragePlanner::SolveLocalCoverageProblem(const exploration_path_ns::ExplorationPath &global_path,
    ...
    misc_utils_ns::Timer find_path_timer("find path");
    find_path_timer.Start();
    // 找出局部规划路径中必有的四个点： 本体位置，目标位置，和全局路径的两个交点
    std::vector<int> navigation_viewpoint_indices;
    ...
    for (auto &viewpoint_array_ind : pre_selected_viewpoint_array_indices) {
        ...
        if (use_frontier_) { UpdateViewPointCoveredFrontierPoint(frontier_covered, viewpoint_array_ind, true); }
    }
    ...
    if (!queue.empty() && queue[0].first > parameters_.kMinAddPointNum) {
        ...
        for (int itr = 0; itr < parameters_.kLocalPathOptimizationItrMax; itr++) {
            ...
            for (const auto &ind : reused_viewpoint_indices) { selected_viewpoint_indices_itr.push_back(ind); }
            // Add viewpoints for navigation
            for (const auto &ind : navigation_viewpoint_indices) { selected_viewpoint_indices_itr.push_back(ind); }
    ...
    for (int i = 0; i < viewpoint_num; i++) { viewpoint_manager_->SetViewPointSelected(i, false, true); }
