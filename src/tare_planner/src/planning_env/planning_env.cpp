...
void PlanningEnvParameters::ReadParameters(ros::NodeHandle &nh) {
  ...
  kKeyposeGraphCollisionCheckRadius = misc_utils_ns::getParam<double>(
      nh, "keypose_graph/kAddEdgeCollisionCheckRadius", 0.4);
  kKeyposeGraphCollisionCheckPointNumThr = misc_utils_ns::getParam<int>(
      nh, "keypose_graph/kAddEdgeCollisionCheckPointNumThr", 1);

  kKeyposeCloudStackNum =
      misc_utils_ns::getParam<int>(nh, "kKeyposeCloudStackNum", 5);
  ...
  kPointCloudLevelNum =
      misc_utils_ns::getParam<int>(nh, "kPointCloudLevelNum", 10);
  kMaxCellPointNum =
      misc_utils_ns::getParam<int>(nh, "kMaxCellPointNum", 100000);
  kPointCloudCellSize =
      misc_utils_ns::getParam<double>(nh, "kPointCloudCellSize", 24.0);
  kPointCloudCellHeight =
      misc_utils_ns::getParam<double>(nh, "kPointCloudCellHeight", 3.0);
  ...
  kFrontierClusterMinSize =
      misc_utils_ns::getParam<int>(nh, "kFrontierClusterMinSize", 30);

  kUseCoverageBoundaryOnFrontier = misc_utils_ns::getParam<bool>(
      nh, "kUseCoverageBoundaryOnFrontier", false);
  kUseCoverageBoundaryOnObjectSurface = misc_utils_ns::getParam<bool>(
      nh, "kUseCoverageBoundaryOnObjectSurface", false);
  ...
  double local_planning_horizon_half_size =
      viewpoint_number * viewpoint_resolution / 2;
  double sensor_range = misc_utils_ns::getParam<double>(nh, "kSensorRange", 15);

  kExtractFrontierRange.x() =
      local_planning_horizon_half_size + sensor_range * 2;
  kExtractFrontierRange.y() =
      local_planning_horizon_half_size + sensor_range * 2;
...
PlanningEnv::PlanningEnv(ros::NodeHandle nh, ros::NodeHandle nh_private,
    ...
    : keypose_cloud_count_(0), vertical_surface_extractor_(),
      vertical_frontier_extractor_(), robot_position_update_(false) {
  ...
  for (int i = 0; i < vertical_surface_cloud_stack_.size(); i++) {
    vertical_surface_cloud_stack_[i].reset(
        new pcl::PointCloud<PlannerCloudPointType>());
  }
  keypose_cloud_ =
  ...
  stacked_vertical_surface_cloud_kdtree_ =
      pcl::KdTreeFLANN<PlannerCloudPointType>::Ptr(
          new pcl::KdTreeFLANN<PlannerCloudPointType>());
  ...
  pointcloud_manager_ =
      std::make_unique<pointcloud_manager_ns::PointCloudManager>(
  ...
  pointcloud_manager_->SetCloudDwzFilterLeafSize() =
      parameters_.kSurfaceCloudDwzLeafSize;

  rolling_occupancy_grid_ =
      std::make_unique<rolling_occupancy_grid_ns::RollingOccupancyGrid>(
          nh_private);

  squeezed_planner_cloud_ =
      std::make_unique<pointcloud_utils_ns::PCLCloud<PlannerCloudPointType>>(
          nh, "squeezed_planner_cloud", world_frame_id);
  squeezed_planner_cloud_kdtree_ = pcl::KdTreeFLANN<PlannerCloudPointType>::Ptr(
      new pcl::KdTreeFLANN<PlannerCloudPointType>());
  ...
  kdtree_frontier_cloud_ = pcl::search::KdTree<pcl::PointXYZI>::Ptr(
  ...
  double vertical_frontier_neighbor_search_radius =
      std::max(rolling_occupancy_grid_resolution.x(),
               rolling_occupancy_grid_resolution.y());
  vertical_frontier_neighbor_search_radius =
      std::max(vertical_frontier_neighbor_search_radius,
               rolling_occupancy_grid_resolution.z());
  vertical_frontier_extractor_.SetRadiusThreshold(
      vertical_frontier_neighbor_search_radius);
...
void PlanningEnv::UpdateCollisionCloud() {
  ...
  for (int i = 0; i < parameters_.kKeyposeCloudStackNum; i++) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_tmp(
  ...
  collision_cloud_downsizer_.Downsize(collision_cloud_,
...
void PlanningEnv::UpdateFrontiers() {
  if (parameters_.kUseFrontier) {
    ...
    rolling_occupancy_grid_->GetFrontier(frontier_cloud_->cloud_,
                                         robot_position_,
    ...
    if (!frontier_cloud_->cloud_->points.empty()) {
      ...
      vertical_frontier_extractor_
          .ExtractVerticalSurface<pcl::PointXYZI, pcl::PointXYZI>(
    ...
    if (!filtered_frontier_cloud_->cloud_->points.empty()) {
      ...
      for (int i = 0; i < cluster_indices.size(); i++) {
        if (cluster_indices[i].indices.size() <
            parameters_.kFrontierClusterMinSize) {
          continue;
        }
...
void PlanningEnv::UpdateTerrainCloud(
...
bool PlanningEnv::InCollision(double x, double y, double z) const {
  if (stacked_cloud_->cloud_->points.empty()) {
    ROS_WARN("PlanningEnv::InCollision(): collision cloud empty, not checking "
             "collision");
    return false;
  }
  ...
  stacked_vertical_surface_cloud_kdtree_->radiusSearch(
      check_point, parameters_.kKeyposeGraphCollisionCheckRadius,
      neighbor_indices, neighbor_sqdist);
  if (neighbor_indices.size() >
      parameters_.kKeyposeGraphCollisionCheckPointNumThr) {
    return true;
  } else {
    return false;
  }
...
void PlanningEnv::UpdateCoveredArea(
  ...
  if (planner_cloud_->cloud_->points.empty()) {
    std::cout << "Planning cloud empty, cannot update covered area"
              << std::endl;
    return;
  }
  ...
  double coverage_dilation_radius =
      viewpoint_manager->GetCoverageDilationRadius();
  ...
  double xy_dist_threshold =
      3 * (parameters_.kSurfaceCloudDwzLeafSize / 2) / 0.3;
  ...
  for (int i = 0; i < planner_cloud_->cloud_->points.size(); i++) {
    ...
    if (std::abs(point.z - robot_position.z) < diff_z_max) {
      if (misc_utils_ns::InFOVSimple(Eigen::Vector3d(point.x, point.y, point.z),
                                     Eigen::Vector3d(robot_position.x,
                                                     robot_position.y,
                                                     robot_position.z),
                                     vertical_fov_ratio, sensor_range,
                                     xy_dist_threshold, z_diff_threshold)) {
  ...
  squeezed_planner_cloud_kdtree_->setInputCloud(
      squeezed_planner_cloud_->cloud_);
  ...
  for (const auto &ind : covered_point_indices) {
    ...
    squeezed_planner_cloud_kdtree_->radiusSearch(
        point, coverage_dilation_radius, nearby_indices, nearby_sqdist);
...
void PlanningEnv::GetUncoveredArea(
  ...
  for (int i = 0; i < planner_cloud_->cloud_->points.size(); i++) {
    ...
    for (const auto &viewpoint_ind : viewpoint_manager->candidate_indices_) {
      if (!viewpoint_manager->ViewPointVisited(viewpoint_ind)) {
        if (viewpoint_manager->VisibleByViewPoint<PlannerCloudPointType>(
                point, viewpoint_ind)) {
          viewpoint_manager->AddUncoveredPoint(viewpoint_ind,
                                               uncovered_point_num);
          observed = true;
        }
      }
  ...
  if (parameters_.kUseFrontier) {
    for (int i = 0; i < filtered_frontier_cloud_->cloud_->points.size(); i++) {
      ...
      for (const auto &viewpoint_ind : viewpoint_manager->candidate_indices_) {
        if (!viewpoint_manager->ViewPointVisited(viewpoint_ind)) {
          if (viewpoint_manager->VisibleByViewPoint<pcl::PointXYZI>(
                  point, viewpoint_ind)) {
      ...
      if (observed) {
        ...
        uncovered_frontier_cloud_->cloud_->points.push_back(
            uncovered_frontier_point);
...
void PlanningEnv::PublishStackedCloud() { stacked_cloud_->Publish(); }

void PlanningEnv::PublishUncoveredCloud() { uncovered_cloud_->Publish(); }
