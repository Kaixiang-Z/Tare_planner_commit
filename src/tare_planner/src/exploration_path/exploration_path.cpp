...
Node::Node() :
    type_(NodeType::LOCAL_VIA_POINT), local_viewpoint_ind_(-1), keypose_graph_node_ind_(-1), global_subspace_index_(-1),
    position_(Eigen::Vector3d::Zero()), nonstop_(false) {
}
...
double ExplorationPath::GetLength() const {
    double length = 0.0;
    if (nodes_.size() < 2) { return length; }
    for (int i = 1; i < nodes_.size(); i++) { length += (nodes_[i].position_ - nodes_[i - 1].position_).norm(); }
    return length;
}

void ExplorationPath::Append(const Node &node) {
    if (nodes_.empty() || nodes_.back() != node) { nodes_.push_back(node); }
}

void ExplorationPath::Append(const ExplorationPath &path) {
    for (int i = 0; i < path.nodes_.size(); i++) { Append(path.nodes_[i]); }
}
...
void ExplorationPath::GetNodePositions(std::vector<Eigen::Vector3d> &positions) const {
    positions.clear();
    for (int i = 0; i < nodes_.size(); i++) { positions.push_back(nodes_[i].position_); }
}
