...
double DistancePoint2DToPolygon(const geometry_msgs::Point &point, const geometry_msgs::Polygon &polygon) {
    ...
    do {
        ...
        if (distance < distance_return) { distance_return = distance; }
...
void ConcatenatePath(nav_msgs::Path &path1, const nav_msgs::Path &path2, int from_ind, int to_ind) {
    ...
    if (from_ind <= to_ind) {
        for (int i = from_ind; i <= to_ind; i++) { path1.poses.push_back(path2.poses[i]); }
    } else {
        for (int i = from_ind; i >= to_ind; i--) { path1.poses.push_back(path2.poses[i]); }
    }
}
...
void RayCast(const Eigen::Vector3i &start_sub, const Eigen::Vector3i &end_sub, const Eigen::Vector3i &max_sub,
    ...
    // 三维空间中的光线投射（Ray Casting）算法，用于确定从一个起点到终点之间的所有整数坐标点
    output.clear();
    ...
    while (true) {
        ...
        if (cur_sub == end_sub || dist > max_dist) { return; }
        if (t_max_x < t_max_y) {
            if (t_max_x < t_max_z) {
                cur_sub.x() += step_x;
                t_max_x += t_delta_x;
...
bool InFOVSimple(const Eigen::Vector3d &point_position, const Eigen::Vector3d &viewpoint_position,
                 double vertical_fov_ratio, double range, double xy_dist_threshold, double z_diff_threshold,
                 bool print) {
    Eigen::Vector3d diff = point_position - viewpoint_position;
    double xy_dist = sqrt(diff.x() * diff.x() + diff.y() * diff.y());
    if (xy_dist < xy_dist_threshold && std::abs(diff.z()) < z_diff_threshold) {
        if (print) { std::cout << "nearby point approximation" << std::endl; }
        return true;
    }
    if (xy_dist > range) {
        if (print) { std::cout << "xy_dist: " << xy_dist << " > range: " << range << std::endl; }
        return false;
    }
...
double GetPathLength(const nav_msgs::Path &path) {
    ...
    if (path.poses.size() < 2) { return path_length; }
...
double GetPathLength(const std::vector<Eigen::Vector3d> &path) {
    // 计算path的长度
    double path_length = 0.0;
    if (path.size() < 2) { return path_length; }
...
bool AStarSearchWithMaxPathLength(const std::vector<std::vector<int>> &graph,
    ...
    // A* 算法，搜索 a 到 b 的最短路经，同时带有最大距离限制
    MY_ASSERT(graph.size() == node_dist.size());
    MY_ASSERT(graph.size() == node_positions.size());
    MY_ASSERT(misc_utils_ns::InRange<std::vector<int>>(graph, from_idx));
    MY_ASSERT(misc_utils_ns::InRange<std::vector<int>>(graph, to_idx));
    ...
    while (!pq.empty()) {
        ...
        // 标记该节点已经不在优先队列中
        in_pg[u] = false;
        // 如果到达终点，更新最短距离并且返回true
        if (u == to_idx) {
            shortest_dist = g[u];
            found_path = true;
            break;
        }
        // 如果距离已经超过了最大距离限制，返回false
        ...
        for (int i = 0; i < graph[u].size(); i++) {
            ...
            if (g[v] > g[u] + d) {
                ...
                prev[v] = u;
                // 更新距离和代价
                // 代价计算方式: v的实际代价 g[v] 加 v到终点的欧氏距离
                g[v] = g[u] + d;
                f[v] = g[v]
                       + misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
                           node_positions[v], node_positions[to_idx]);
                // 如果 v 已经不在graph中， 重新加入
                if (!in_pg[v]) {
                    pq.push(std::make_pair(f[v], v));
                    in_pg[v] = true;
                }
...
nav_msgs::Path DeduplicatePath(const nav_msgs::Path &path, double min_dist) {
    ...
    if (dist_to_prev > min_dist && dist_to_start > min_dist) { deduplicated_path.poses.push_back(path.poses.back()); }
    return deduplicated_path;
}
