...
void KeyposeToMap(CloudType &cloud,
...
double PointXYDist(const geometry_msgs::Point &pnt1,
                   const geometry_msgs::Point &pnt2) {
...
/// Function to compute the angle between two vectors in the xy-plane, the z
/// component is omitted \param v1 The first (starting) vector \param v2 The
/// second (end) vector \return Angle from v1 to v2, positive if
/// counterclockwise, negative if clockwise. The output is within [-pi, pi]
double VectorXYAngle(const Eigen::Vector3d &v1, const Eigen::Vector3d &v2) {
  return atan2(v1.x() * v2.y() - v1.y() * v2.x(),
               v1.x() * v2.x() + v1.y() * v2.y());
}
...
double PointAngle(const geometry_msgs::Point &pnt,
                  const geometry_msgs::Point &robot_pos) {
...
/// \param robot_pos Robot position
/// \return Direction (angle)
double PointAngle(const PCLPointType &pnt,
                  const geometry_msgs::Point &robot_pos) {
  return atan2((pnt.y - robot_pos.y), (pnt.x - robot_pos.x));
}
...
/// Function to calculate the overlap of two angle intervals [s1, e1] and [s2,
/// e2]. s1(2)->e1(2) counter-clockwise and all angles shoud be in [-pi, pi]
/// \param s1 starting angle of the first interval \param e1 end angle of the
/// second interval \param s2 starting angle of the first interval \param e2 end
/// angle of the second interval \return overlap the overlap of the two
/// intervals. > 0 if overlapped, < 0 otherwise.
double AngleOverlap(double s1, double e1, double s2, double e2) {
  ...
  if (e1 < s1) {
    ...
    // If both sub-overlaps are negative (no overlap) or there is only one
    // positive sub-overlap
    if ((sub_overlap1 < 0 && sub_overlap2 < 0) ||
        sub_overlap1 * sub_overlap2 < 0) {
...
/// line segment 'pr' \param p End point of line segment pr \param q Point to be
/// examined \param r End point of line segment pr \return If q is on pr
bool PointOnLineSeg(const geometry_msgs::Point &p,
...
/// \return 0 --> p, q and r are colinear, 1 --> Clockwise, 2 -->
/// Counterclockwise
...
bool LineSegIntersect(const geometry_msgs::Point &p1,
...
bool PointInPolygon(const geometry_msgs::Point &point,
  ...
  do {
    ...
    geometry_msgs::Point polygon_cur_pnt =
        GeoMsgPoint(polygon.points[cur_idx].x, polygon.points[cur_idx].y,
                    polygon.points[cur_idx].z);
    geometry_msgs::Point polygon_next_pnt =
        GeoMsgPoint(polygon.points[next_idx].x, polygon.points[next_idx].y,
                    polygon.points[next_idx].z);
    if (LineSegIntersect(polygon_cur_pnt, polygon_next_pnt, point, inf_point)) {
      ...
      // otherwise false
      if (ThreePointOrientation(polygon_cur_pnt, point, polygon_next_pnt) ==
          0) {
...
/// Function to get distance from a point to the closest point on boundary of a
/// polygon \param p point \param polygon polygon \return distance
double DistancePoint2DToPolygon(const geometry_msgs::Point &point,
  ...
  do {
    ...
    double distance =
        LineSegDistance2D(point, polygon_cur_pnt, polygon_next_pnt);
...
void LinInterpPoints(const Eigen::Vector3d &p1, const Eigen::Vector3d &p2,
                     double resolution,
...
void ConcatenatePath(nav_msgs::Path &path1, const nav_msgs::Path &path2,
                     int from_ind, int to_ind) {
...
void SetDifference(std::vector<int> &v1, std::vector<int> &v2,
                   std::vector<int> &diff) {
...
void RayCast(const Eigen::Vector3i &start_sub, const Eigen::Vector3i &end_sub,
  ...
  double t_max_x =
      step_x == 0 ? DBL_MAX : intbound(start_sub.x(), diff_sub.x());
  double t_max_y =
      step_y == 0 ? DBL_MAX : intbound(start_sub.y(), diff_sub.y());
  double t_max_z =
      step_z == 0 ? DBL_MAX : intbound(start_sub.z(), diff_sub.z());
  double t_delta_x =
      step_x == 0 ? DBL_MAX : (double)step_x / (double)diff_sub.x();
  double t_delta_y =
      step_y == 0 ? DBL_MAX : (double)step_y / (double)diff_sub.y();
  double t_delta_z =
      step_z == 0 ? DBL_MAX : (double)step_z / (double)diff_sub.z();
  double dist = 0;
...
bool InFOV(const Eigen::Vector3d &point_position,
           const Eigen::Vector3d &viewpoint_position, double vertical_half_fov,
           double range) {
...
bool InFOVSimple(const Eigen::Vector3d &point_position,
                 ...
                 double vertical_fov_ratio, double range,
                 double xy_dist_threshold, double z_diff_threshold,
                 bool print) {
  Eigen::Vector3d diff = point_position - viewpoint_position;
  ...
  for (int i = 0; i < path.poses.size() - 1; i++) {
    ...
    path_length +=
        misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
            path.poses[cur_pose_idx].pose.position,
            path.poses[next_pose_idx].pose.position);
  }
  return path_length;
}
  ...
  while (!pq.empty()) {
    ...
    for (int i = 0; i < graph[u].size(); i++) {
      ...
      if (g[v] > g[u] + d) {
        prev[v] = u;
        g[v] = g[u] + d;
        f[v] = g[v] + misc_utils_ns::PointXYZDist<geometry_msgs::Point,
                                                  geometry_msgs::Point>(
...
bool AStarSearchWithMaxPathLength(
    ...
    double &shortest_dist, double max_path_length) {
  MY_ASSERT(graph.size() == node_dist.size());
  MY_ASSERT(graph.size() == node_positions.size());
  MY_ASSERT(misc_utils_ns::InRange<std::vector<int>>(graph, from_idx));
  MY_ASSERT(misc_utils_ns::InRange<std::vector<int>>(graph, to_idx));
...
void SampleLineSegments(const std::vector<Eigen::Vector3d> &initial_points,
                        double sample_resol,
...
template void misc_utils_ns::KeyposeToMap<pcl::PointCloud<pcl::PointXYZI>::Ptr>(
    ...
    const nav_msgs::Odometry::ConstPtr &keypose);
template void
misc_utils_ns::KeyposeToMap<pcl::PointCloud<pcl::PointXYZINormal>::Ptr>(
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &cloud,
    const nav_msgs::Odometry::ConstPtr &keypose);
template void
misc_utils_ns::KeyposeToMap<pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr>(
