...
LiDARModel::LiDARModel(double px, double py, double pz, double rw, double rx,
                       double ry, double rz) {
  pose_.position.x = px;
  pose_.position.y = py;
  pose_.position.z = pz;
...
LiDARModel::LiDARModel(const geometry_msgs::Pose &pose)
    : LiDARModel(pose.position.x, pose.position.y, pose.position.z,
...
void LiDARModel::GetVisualizationCloud(
    pcl::PointCloud<pcl::PointXYZI>::Ptr &visualization_cloud, double resol,
  ...
  for (int i = 0; i < covered_voxel_.size(); i++) {
    ...
    misc_utils_ns::LinInterpPoints<pcl::PointXYZI>(start_point, end_point,
                                                   resol, tmp_cloud);
